...
import com.moreo.shorlink.project.dao.entity.LinkDeviceStatsDO;
import com.moreo.shorlink.project.dao.entity.LinkLocaleStatsDO;
import com.moreo.shorlink.project.dao.entity.LinkNetworkStatsDO;
...
public class ShortLinkStatsServiceImpl implements ShortLinkStatsService {
    ...
    public ShortLinkStatsRespDTO oneShortLinkStats(ShortLinkStatsReqDTO requestParam) {
        ...
        List<String> rangeDates = DateUtil.rangeToList(DateUtil.parse(requestParam.getStartDate()), DateUtil.parse(requestParam.getEndDate()), DateField.DAY_OF_MONTH).stream()
                .map(DateUtil::formatDate).toList();
        // 计算每一天的数据
        rangeDates.forEach(each -> listStatsByShortLink.stream()
                .filter(item -> Objects.equals(each, DateUtil.formatDate(item.getDate())))
                .findFirst()
                .ifPresentOrElse(item -> {
                    ShortLinkStatsAccessDailyRespDTO dailyResp = ShortLinkStatsAccessDailyRespDTO.builder()
                            .date(each)
                            .pv(item.getPv())
                            .uv(item.getUv())
                            .uip(item.getUip())
        ...
        listTopIpByShortLink.forEach(each -> {
            ShortLinkStatsTopIpRespDTO statsTopIpRespDTO = ShortLinkStatsTopIpRespDTO.builder()
                    ...
                    .build();
            topIpRespDTOS.add(statsTopIpRespDTO);
        });
        // 一周访问详情
        ...
        HashMap<String, Object> findUvTypeByShortLink = linkAccessStatsMapper.findUvTypeCntByShortLink(requestParam);
